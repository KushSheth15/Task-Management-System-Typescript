...
export const registerUser = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (existingUser) {
        return next(
          new ApiError(400, ERROR_MESSAGES.EMAIL_OR_USERNAME_EXISTS),
        );
      }
...
export const loginUser = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!email || !password) {
      return next(
        new ApiError(400, ERROR_MESSAGES.EMAIL_AND_PASSWORD_REQUIRED),
      );
    }
    ...
    try {
      ...
      if (!isMatch) {
        return next(
          new ApiError(401, ERROR_MESSAGES.INVALID_CREDENTIALS),
        );
      }
  ...
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const response = new ApiResponse(
        200,
        null,
        SUCCESS_MESSAGES.USER_LOGOUT_SUCCESSFULLY,
      );
      res.status(200).json(response);
    } catch (error) {
      console.error(error);

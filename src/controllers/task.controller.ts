    ...
    try {
      ...
      if (cachedTasks) {
        ...
        const response = new ApiResponse(
          200,
          tasks,
          SUCCESS_MESSAGES.TASKS_RETRIEVED_SUCCESSFULLY,
        );
  ...
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const response = new ApiResponse(
        200,
        updateTask,
        SUCCESS_MESSAGES.TASK_UPDATED_SUCCESSFULLY,
      );
      res.status(200).json(response);
    } catch (error) {
      console.error(error);
...
export const shareTask = asyncHandler(
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!taskId || !userId) {
        return next(
          new ApiError(400, ERROR_MESSAGES.ALL_FIELDS_REQUIRED),
        );
      }
      ...
      if (existingShare) {
        return next(
          new ApiError(400, ERROR_MESSAGES.TASK_ALREADY_SHARED),
        );
      }
      ...
      const response = new ApiResponse(
        201,
        taskShare,
        SUCCESS_MESSAGES.TASK_SHARED_SUCCESSFULLY,
      );
      res.status(201).json(response);
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      res.json(
        new ApiResponse(
          200,
          task,
          SUCCESS_MESSAGES.TASK_MOVED_SUCCESSFULLY,
        ),
      );
    } catch (error) {
      console.error(error);
...
export const getTaskByStatus = asyncHandler(
...
export const getFilteredTasks = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const {
        statusId,
...
export const bulkCreateTasks = asyncHandler(
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const createdTasks = await db.Task.bulkCreate(tasksWithUserId, {
        returning: true,
      });
      res.json(
        new ApiResponse(
...
export const bulkAssignTask = asyncHandler(
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    const user = req.user;
    if (!user) {
      return next(new ApiError(401, ERROR_MESSAGES.UNAUTHORIZED_USER));
    ...
    try {
      ...
      if (!Array.isArray(taskAssignment) || taskAssignment.length === 0) {
        return next(
          new ApiError(
            400,
            ERROR_MESSAGES.NO_TASK_ASSIGNMENTS_PROVIDED,
          ),
        );
      }
      ...
      taskAssignment.forEach((assignment: any) => {
        if (!assignment.taskId || !assignment.userId) {
          throw new ApiError(
            400,
            ERROR_MESSAGES.MISSING_TASK_ID_OR_USER_ID,
          );
        }
      });
      ...
      res.status(200).json(
        new ApiResponse(
          200,
          null,
          SUCCESS_MESSAGES.TASKS_ASSIGNED_SUCCESSFULLY,
        ),
      );
    } catch (error) {
      console.error(error);
...
export const bulkDeleteTasks = asyncHandler(
  async (req: MyUserRequest, res: Response, next: NextFunction) => {
    const user = req.user;
    if (!user) {
      return next(new ApiError(401, ERROR_MESSAGES.UNAUTHORIZED_USER));

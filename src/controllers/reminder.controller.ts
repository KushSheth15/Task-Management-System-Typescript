...
export const createReminder = asyncHandler (async(req:MyUserRequest,res:Response,next:NextFunction)=>{
    ...
    try {
        const task = await db.Task.findByPk(taskId,{
            include: [
                {model:db.TaskShare,as:'sharedUsers',
                    include:[
        ...
        const response = new ApiResponse(201,newReminder,SUCCESS_MESSAGES.REMINDER_CREATED);
...
export const updateReminder = asyncHandler(async (req: MyUserRequest, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { taskId, reminderDate } = req.body;
    const user = req.user;
    ...
    try {
        ...
        if (!reminder) {
            return next(new ApiError(404, ERROR_MESSAGES.REMINDER_NOT_FOUND));
        }
        ...
        const response = new ApiResponse(200, reminder, SUCCESS_MESSAGES.REMINDER_UPDATED);
